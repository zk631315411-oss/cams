...
DATA = BASE / "data"
# KG files not yet rebuilt in new workspace; point to old for now
KG_DIR = Path(r"D:\守正公司工作区\cams考试\cams工作台\data\agentic_search_eval_v2\kg")
SAVE_DIR = BASE / "tools" / "选项证据生成" / "output" / "step1_ai_responses"
EVIDENCE_FILES = {
...
MD_DIR = Path(r"D:\守正公司工作区\cams考试\教材、答疑记录、习题与参考文献\习题\习题结构化")


def _load_questions_from_md() -> list[dict[str, Any]]:
    """从习题结构化 MD 加载全教材题目（替代旧 questions.json）。"""
    _RE_Q = re.compile(r"^##\s*第(\d+)题\s*(.+)")
    _RE_O = re.compile(r"^-\s*([A-K])[\.\、\)）]\s*(.+)", re.MULTILINE)
    _RE_A = re.compile(r"答案\s*[:：]\s*([A-K,，、/;；\s]+)")
    sect_re = re.compile(r"^#\s*(?:第[一二三四五六七八九十]+章\s*)?([\d.]+)\s*习题(?:集)?")
    ...
    for fname in sorted(os.listdir(MD_DIR)):
        ...
        sm = sect_re.search(text)
        section = sm.group(1).strip() if sm else fname.split("_")[0]
        ...
        for block in blocks:
            ...
            opts = {om.group(1): om.group(2).strip() for om in _RE_O.finditer(block)}
            am = _RE_A.search(block)
            ans = ",".join(sorted(set(re.findall(r"[A-K]", am.group(1).upper())))) if am else ""
            qid = f"{section}_{num}"
            all_qs.append({"id": qid, "section": section, "number": num, "stem": stem, "options": opts, "answer": ans})
    return all_qs


def load_runtime(evidence_scope: str = "v6-sentence") -> Runtime:
    ...
    questions = _load_questions_from_md()
    ...
    try:
        ...
        # sentence-transformers 2.x does not expose local_files_only on the
        # public constructor. Offline env vars above still force local cache use.
...
def bge_search(rt: Runtime, query_text: str, top_k_sections: int = 5, top_k_cards: int = 3) -> tuple[list[dict[str, Any]], list[str], list[str]]:
    ...
    card_scores = (q_vec @ rt.card_vecs.T).flatten()
    ...
    # The KG section-card map still belongs to the legacy v6_b card space.
    # Search the active evidence card pool directly so v6-sentence runs only
    # expose real v6s_N... card ids to the adjudicator.
    top_n_cards = max(top_k_sections * top_k_cards * 2, 12)
    for card_idx in list(reversed(card_scores.argsort()))[:top_n_cards]:
        cid = rt.card_ids[int(card_idx)]
        if cid in seen_cids:
            continue
        seen_cids.add(cid)
        card = rt.cards[int(card_idx)] if int(card_idx) < len(rt.cards) else {}
        section = card.get("chapter_path") or card.get("section") or ""
        evidence.append(
            {
                "card_id": cid,
                "section": section,
                "bge_score": float(card_scores[card_idx]),
                "source": "card_bge_direct",
...
def call_llm(client: Any, prompt: str, max_tokens: int, retries: int = LLM_RETRY) -> str:
...
{{
  "option_analysis": [
    {{
      ...
      "evidence_cards": [
        {{"card_id": "必须使用上方证据中真实出现的card_id", "support_type": "direct/indirect/context/negative", "source": "card_bge_direct", "quote": "教材原文短摘，不超过120字", "reason": "为什么支撑该选项", "relevance": "high/medium/low"}}
      ],
...
def is_valid_done_file(
    ...
    if data.get("retrieval_mode", "baseline") != retrieval_mode:
        return False
    if data.get("evidence_scope", "v6-sentence") != evidence_scope:
        return False
