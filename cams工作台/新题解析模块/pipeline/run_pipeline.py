...
from pipeline.evidence_pool import get_agentic_runtime  # noqa: E402
...
def _finalize_answer_by_type(
    ...
    # Detect conflict: same card_id used to support opposite options.
    # Only enforce for true/false questions (2 options) — for multi-select,
    # one card can legitimately support multiple non-exclusive options.
    if len(options) == 2:
        _card_option_map: dict[str, set[str]] = {}
        for row in option_analysis:
            label = str(row.get("option", "")).strip().upper()
            for ec in row.get("evidence_cards", []) or []:
                cid = ec.get("card_id", "")
                if cid:
                    _card_option_map.setdefault(cid, set()).add(label)
        _conflict_options: set[str] = set()
        for cid, labels in _card_option_map.items():
            if len(labels) > 1:
                _conflict_options.update(labels)
        if _conflict_options:
            for row in option_analysis:
                label = str(row.get("option", "")).strip().upper()
                if label in _conflict_options and row.get("judgement") == "correct":
                    row["judgement"] = "conflict"
                    row["evidence_status"] = "conflict"
                    row["judgement_confidence"] = "low"
                    row.setdefault("teacher_review_reason", "")
                    if not row.get("teacher_review_reason"):
                        row["teacher_review_reason"] = "同一张句卡被用于支撑互斥选项，自动标记为冲突"

    direct_correct = _direct_correct_labels(option_analysis, options)

    if question_type in {"true_false"}:
        # True/False: at most one correct, and only if evidence is direct
        if len(direct_correct) == 1:
            final = direct_correct
            status = "ok"
        elif len(direct_correct) > 1:
            final = predicted[:1] if predicted else []
            status = "conflict_multiple_correct"
        elif predicted:
            final = predicted[:1]
        ...
        if direct_correct:
            # Cap at 3 for safety — over-selection is worse than under-selection
            merged = list(dict.fromkeys(direct_correct + predicted))
            final = merged[:3] if len(merged) > 3 else merged
            status = "ok"
        elif predicted:
            final = predicted[:3]
            status = "model_only_needs_review"
        ...
        if len(direct_correct) == 1:
            ...
            # Multiple options with direct evidence for a single-answer question:
            # cannot safely tie-break — flag all for review.
            final = []
            status = "conflict_multiple_correct"
