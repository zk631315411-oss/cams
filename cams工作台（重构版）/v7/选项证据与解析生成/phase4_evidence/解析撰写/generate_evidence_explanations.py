...
6. 区分三种作答方式：
   a. 材料有直接覆盖 → 引用证据 + 解释
   b. 材料无直接覆盖但可通过题干逻辑推理判断（如排除法、定义对比、场景匹配）→ 基于推理作答，标注"此题主要通过题干逻辑推理得出，教材材料未直接覆盖该概念"
   c. 材料无覆盖且推理也无法判断 → 才可拒答
   当材料没有提供某个概念的定义时，先问自己：能否通过排除错误的选项来确定答案？如果能，用排除法作答，不需要为每个选项都找教材证据。
...
14. **拒答仅用于完全无法判断**：只有同时满足以下两个条件时才拒答：(a) 材料无直接覆盖，(b) 也无法通过题干逻辑推理（排除法、定义对比、场景匹配）得出答案。只要有合理的推理路径（即使证据不完美），就基于推理作答并标注。拒答比推理作答更差——它意味着这道题完全无法使用。正常解析时 deferral 字段留空（设置为 null）。

15. **禁止空口说"无教材引用"**：在声称"无教材引用"或"不足以判断"之前，必须先检查：候选池中是否存在与题干主题词（或选项关键词）相关、即使只是部分相关的 unit？如有，必须引用并说明其覆盖了什么、未覆盖什么。只有当候选池中确实没有任何相关 unit 时，才可写"本题依据题干事实推导"。擅自跳过池中已有证据、直接宣称无引用，属于证据遗漏错误。
