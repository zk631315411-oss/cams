...
题干请求的是有关客户的「更多相关信息」，同时提示纽约办事处曾提交SAR。判断前应区分**底层客户/交易信息与SAR本身或其存在事实**。教材直接指出，数据隐私法可能限制个人数据跨境传输，除非满足相应条件，金融机构必须确保跨境传输不违反适用法律（P209）。因此，马德里与纽约之间分享哪些客户信息，必须结合相关司法管辖区的隐私要求及银行内部的权限和程序判断，C成立。
