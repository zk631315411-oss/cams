...
核心解析：质量控制的一个主要目标是「通过识别不完整、不正确或过时的KYC记录来检测和纠正错误」（P313）。题干描述的场景——高风险客户未按内部规定的时间表进行审查——正属于KYC记录/流程中的「不正确」或遗漏，应优先通过质量控制机制来识别原因并纠正。选项B「评估KYC审查流程以了解为何审查未按要求进行，并在必要时采取纠正措施」直接对应了「检测和纠正错误」这一核心步骤：先找到流程哪里出了问题（评估），再修复它（纠正措施）。相比之下，立即报告SAR、冻结账户或移除风险评级都是在问题根因未明时的过度反应。
教材原句："Error detection and correction by identifying incomplete, incorrect, or outdated KYC records."

A项错误：提交SAR的前提是「发现并确认可疑活动可能涉及洗钱」（P329）。题干只描述了审查遗漏，并未出现任何可疑交易或洗钱迹象，此时提交SAR的条件不满足，不如优先纠正内部流程缺陷更直接匹配问题本质。
C项错误：「定期KYC指按固定间隔更新客户信息」（P374），是持续尽职调查的一环。题干的问题是内部流程未执行，而非客户本身有可疑行为需紧急控制。在未查明原因前直接暂停账户访问，措施力度与当前已知事实不匹配，优先排查内部流程更符合教材对质量控制的定位。
D项错误：风险评级是银行基于客户尽职调查结果分配的属性（P466）。审查被遗漏是银行内部操作的问题，与客户本身的风险状况无关。因内部流程失误而移除高风险评级，混淆了流程错误与风险判断，更不吻合教材对风险评级的持续审查要求。
