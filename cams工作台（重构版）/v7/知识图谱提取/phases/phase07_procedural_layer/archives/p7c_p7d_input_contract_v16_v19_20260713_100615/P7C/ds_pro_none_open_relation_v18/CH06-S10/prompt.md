...
单一路径的`if/when/unless A，则B`不是分支，不使用`DECIDES`；A可以作为条件entry通过`PRECEDES`进入B，并在edge的`condition`中逐字保留条件。这里的`PRECEDES`表示逻辑前提，未必表示钟表式先后。只有原文明示至少两条不同路径时才使用`P3_branch_routing + DECIDES`。
