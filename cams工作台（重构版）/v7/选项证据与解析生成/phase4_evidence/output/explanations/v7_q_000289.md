# v7_q_000289

教材章节：未映射

题型：multiple

题干：[洗钱和恐怖融资的风险及方法]以下哪三项属于基于风险的方法？(选择三项.)

英文题干：Which of the following are part of a risk-based approach? (Select Three.)

选项：

- A. 根据客户的活动和关系确定其详细的风险特征
  English: Determining detailed risk profiles for customers based on their activities and relationships.
- B. 对于之前已标记的客户,重点进行监控而对于其他客户则采用标准控制措施
  English: Focusing monitoring primarily on previously flagged customers while using standard controls for others.
- C. 选择并应用与所识别风险等级相适应的有效控制措施
  English: Choosing and applying effective controls that align with the identified risk levels.
- D. 进行全面的风险评估,以识别客户、交易和地域风险
  English: Performing a comprehensive risk assessment to identify customer, transaction, and geographic risks.
- E. 在所有客户群体之间平均分配资源以确保公平性
  English: Allocating resources evenly among all customer segments to ensure fairness.

## 【AI答案】

A、C、D

## 【考点】

风险为本方法的核心是识别、评估风险并采取相称的缓解措施

## 【核心解析】

风险为本方法（RBA）是指识别、评估和理解组织面临的洗钱/恐怖融资风险，并采取适当措施有效缓解这些风险（P270）。其核心逻辑是“先评估风险，再按风险等级分配资源和控制”。选项A“根据客户的活动和关系确定**详细风险特征**”对应风险评估中的客户风险画像（P301），属于识别和评估环节；选项D“进行全面的风险评估以识别客户、交易和地域风险”直接对应RBA定义中的识别和评估风险；选项C“选择并应用与所识别风险等级相适应的有效控制措施”对应RBA中“采取与风险成比例的措施”这一关键原则（P254），以及“应用控制措施将固有风险降至可接受的剩余风险水平”。三项共同构成RBA的完整动作链：评估风险 → 按风险等级匹配控制措施。

教材原句："A risk-based approach (RBA) is the process of identifying, assessing, and understanding the ML/TF risks to which organizations are exposed and taking appropriate measures to mitigate those risks effectively."（P270）

## 【错误项分析】

- **B 错误（风险依据片面）**：风险为本方法要求根据客户、交易、地域等多维风险特征动态分配监控资源，而非仅依赖“之前已标记”这一历史状态。教材指出适当的分群确保对不同客户群体进行适当监控，而非只关注已标记客户。以历史标记替代风险评级，无法捕捉尚未被标记但风险正在上升的客户，与RBA的主动、全面评估原则匹配度较低。
- **E 错误（未体现风险差异）**：风险为本方法的核心是按风险高低分配资源，高风险客户接受强化尽职调查和加强监控，从而有效分配资源。在所有客户群体间平均分配资源，违背了RBA“措施与风险成比例”的基本原则，无法体现不同客户群体的风险差异。

## 【易错提醒】

风险为本方法容易与“平均主义”或“仅凭历史标记”混淆。教材明确：RBA要求基于当前风险评估差异化配置资源（P270），而非一刀切平均分配，也不能仅靠过去标记来决定监控强度。

## 【教材原文依据】

> 核心引用单元：`v7u_N002672`

### `v7u_N002672`

- 用于：核心解析、易错提醒
- 章节：Types of risk assessment > The risk-based approach
- 页码：PDF第275页 / 书内第270页
- 中文要点：风险为本方法（RBA）定义：识别、评估和理解洗钱/恐怖融资风险并采取适当缓解措施
- 英文原文：A risk-based approach (RBA) is the process of identifying, assessing, and understanding the ML/TF risks to which organizations are exposed and taking appropriate measures to mitigate those risks effectively.

### `v7u_N003001`

- 用于：核心解析
- 章节：Governance and oversight > Controls across the customer lifecycle
- 页码：PDF第306页 / 书内第301页
- 中文要点：根据交易行为、行业类型和司法管辖区风险对客户进行风险评级
- 英文原文：Risk profiling: Assess customer risk based on transaction behavior, industry type, and jurisdictional exposure to assign a risk rating.

### `v7u_N002506`

- 用于：核心解析
- 章节：Three lines of defense > Case example: Financial crime functions' structure at Global Finance, Corp.
- 页码：PDF第259页 / 书内第254页
- 中文要点：风险评估：进行客户和交易风险评估，识别高风险客户和交易，并应用强化尽职调查（EDD）
- 英文原文：Risk assessment: Conduct customer and transaction risk assessments, identify high-risk clients and transactions, and apply enhance due diligence (EDD).

## 【参考答案与参考解析】

- 题库最终参考答案：A、C、D
- 中文参考答案：A、C、D

### 中文参考解析

基于风险的方法强调识别、评估并应对风险.选 项A根据客户活动确定风险特征,符合风险识 别;选项C选择与风险等级相适应的控制措施 是风险应对:选项D全面评估风险以识别客户、 交易和地域风险,是风险评估.三者均体现基于 风险的方法.选项B虽涉及风险监控,但更侧重 于客户分类管理,未全面体现风险评估、识别和 应对;选项E平均分配资源,未考虑风险差异, 不符合基于风险的方法.因此,正确选项为 ACD.易错提醒:注意区分风险分类管理与全面 风险评估.

- 英文参考答案：A、C、D

### 英文参考解析

风险为本方法要求金融机构根据风险状况采取相 应措施.选项A,基于客户活动和关系确定详细 风险画像,是风险评估的重要环节,符合风险为 本原则.选项C,选择并应用与已识别风险水平 相匹配的有效控制措施,是风险应对的关键步 骤.选项D,进行全面风险评估以识别客户、交 易和地域风险,是风险为本方法的基础.而选项 B侧重于已标记客户,忽视全面监控;选项E平 均分配资源,未考虑风险差异,均不符合风险为 本方法.因此,正确答案为ACD.

### 答案冲突提示

- 未发现答案冲突。
