"""
Build second-chapter textbook sentence cards from v6教材原文/ch2_extracted.md.

The script owns sentence boundaries locally. DeepSeek Flash only labels each
source sentence with a searchable knowledge summary and type. The citation is
always copied from the source sentence verbatim.
"""
...
SOURCE = ROOT / "教材、答疑记录、习题与参考文献" / "教材原文" / "v6" / "ch2_extracted.md"
DEFAULT_OUTPUT = DATA / "cards_ch2.json"
WORK_DIR = BASE / "output" / "ch2_sentence_cards"
INPUT_PATH = WORK_DIR / "sentence_inputs.json"
BATCH_DIR = WORK_DIR / "batches"
STAGING_OUTPUT = WORK_DIR / "cards_ch2_rebuilt.json"
AUDIT_OUTPUT = WORK_DIR / "audit_report.md"
...
def split_sentences_from_line(text: str) -> list[str]:
    ...
    # Inline bullets and numbered lists can appear after a colon. Treat them as
    # independent boundaries so lists do not become one oversized citation.
    text = re.sub(r"(?<!^)\s*[•●○]\s*", "\n• ", text)
...
def build_sentence_inputs(source: Path) -> list[dict[str, Any]]:
...
def build_prompt(batch: list[dict[str, Any]]) -> str:
    ...
    return f"""你是CAMS考试教材句卡标注员。下面每条都是已经切好的第二章教材原文句子。
...
4. 如果原句只是目录、页码、版权、残缺标题或没有教材证据价值，keep=false。
...
def load_or_create_inputs(source: Path, input_path: Path, force_inputs: bool) -> list[dict[str, Any]]:
    if input_path.exists() and not force_inputs:
        return json.loads(input_path.read_text(encoding="utf-8"))
    input_path.parent.mkdir(parents=True, exist_ok=True)
    rows = build_sentence_inputs(source)
...
def merge_outputs(rows: list[dict[str, Any]], batch_dir: Path, source: Path) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        card_id = f"ch2s_{len(cards) + 1:05d}"
        cards.append(
            {
                "card_id": card_id,
                "knowledge": knowledge,
                ...
                "evidence_scope": "ch2_sentence",
            }
        )

    return {
        "asset_note": (
            "Second-chapter CAMS v6.51 textbook fine-grained sentence cards rebuilt from "
            "v6教材原文/ch2_extracted.md. Each citation is an exact source sentence; "
            "DeepSeek Flash only generated knowledge/type labels. This is textbook evidence, "
            "not confirmed exam-point data. Downstream question/exam-point mappings must be rerun."
        ),
        "schema_version": "ch2_sentence_cards_v1",
...
def audit_cards(data: dict[str, Any]) -> tuple[dict[str, Any], str]:
    ...
    for card in cards:
        ...
        if any(token in citation for token in ("![image]", "<table", "頁面", "页面", "......")):
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Build second-chapter CAMS sentence cards.")
    ...
    parser.add_argument("--limit", type=int, default=0, help="Only process the first N sentence inputs.")
    ...
    parser.add_argument("--replace-final", action="store_true", help="Replace cams工作台/data/cards_ch2.json after generation.")
    args = parser.parse_args(argv)

    input_path, batch_dir = scoped_paths(args.work_dir, args.sample_name)
    rows = load_or_create_inputs(args.source, input_path, force_inputs=args.force_inputs)
    ...
    data = merge_outputs(selected_rows, batch_dir=batch_dir, source=args.source)
