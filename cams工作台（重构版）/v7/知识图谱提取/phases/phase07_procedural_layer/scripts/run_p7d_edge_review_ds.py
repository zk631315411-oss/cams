...
#
# 原因：
# 1. P7D 的"原文明示"审核标准与 merged 模式下 S2 的语义建模能力不匹配。
#    S2 正确识别了逻辑先后/参照/产出关系，但教材语言极少逐字标注这些
#    关系（如不写"first/then/based on/produces"），导致大量正确边被判
#    llm_inference→pending 或被拒，pending+rejected 占比 ~50%。
# 2. P7D 在旧流程（S2 KG裁决 + S3 LLM构图）中是必要的——旧 LLM 容易把
#    相关写成因果。merged 模式 S2 已用 role+kind+endpoint matrix 做了
#    结构化的语义把关，P7D 的逐边审核变成了对同一判断的重复且更严格的
#    二次审查。
# 3. 在确定 P7D 的审核标准和 merged 模式的语义标准如何对齐之前，暂时
#    跳过 P7D，直接用 S3 产出的 cards.raw.json 作为 P7C 最终产物。
#
# 恢复条件：重新定义 P7D 的审核口径，使其接受 S2 已建模的逻辑关系，
#   或调整为仅审核"原文明确矛盾"的边（原文明示反向/不存在的关系）。
# ═══════════════════════════════════════════════════════════════════════
