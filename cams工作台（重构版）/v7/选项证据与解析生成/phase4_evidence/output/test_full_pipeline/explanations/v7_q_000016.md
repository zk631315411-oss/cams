...
- **A 错误（题干对照）｜主体或阶段错配**：支付相关的制裁违规防范确实属于制裁合规的一部分（P317），但题干问的是进行「名称筛查」（name screening systems）使用多份名单的原因，而非支付筛查的目的。名称筛查发生在客户准入和定期审查环节（P305），而支付筛查是交易层面的控制措施（P317），两者的业务阶段和筛查对象不同。题干语境更直接对应C的跨辖区合规逻辑，而非A的支付场景。
- **B 错误（题干对照）｜概念混淆**：内部名单（如灰名单）确实可以记录因金融犯罪被退出的客户，用于防止重新接纳（P422），但这属于机构内部维护的名单，与题干中联合国、OFAC、欧盟等外部制裁名单在来源和功能上不同。外部制裁名单的核心目的远大于管理既往退出客户，更直接指向满足国际合规和识别跨区域风险。
- **D 错误（题干对照）｜题干要素不匹配**：风险偏好声明（RAS）定义了机构可接受的风险水平（P271），制裁筛查作为控制措施应与风险偏好保持一致（P277）。但题干问的是「为什么使用多份外部制裁名单很重要」，这是一个关于制裁筛查制度设计动机的问题，而非解释内部控制措施如何与内部风险偏好文件对齐。C直接回答了多名单存在的制度逻辑，更贴合教材对制裁名单来源和目的的描述。

## 【易错提醒】

名称筛查（客户准入/定期审查阶段，P305）与支付筛查（交易处理阶段，P317）是不同环节的控制措施。名称筛查关注用户身份是否命中制裁名单，支付筛查关注交易本身是否违反制裁。题干提及多份国际制裁名单，问的是名称筛查层面为何使用多个来源，不要与支付筛查的目的混淆。
