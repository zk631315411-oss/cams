...
def build_prompt(
    ...
    return f"""你是CAMS教研解析编辑。请根据已锁定答案、题干事实和限定证据，生成V3.1结构化解析材料。你不负责重新判题，也不负责重新检索。

一、硬约束
1. 固定答案是”{predicted}”。不得改判；每个选项的最终正误由固定答案决定。
2. 参考答案、参考解析和外部知识不在输入中，也不得猜测。
3. 有教材证据的选项使用该选项的”已裁判证据”或”解析补充候选”；文本中引用框架定义时可使用”共享框架证据”。不得借用其他选项材料。
4. stem_contrast类型必须输出逐字存在的stem_quotes和option_quotes。不得把题干未提及某事自动等同于该事不存在。
5. 解析补充候选只用于补足说明，不得据此改变固定答案。候选与选项没有直接关系时不得引用。
6. 当有一句教材英文原文能精准概括核心判断时，可将其 40 至 240 字符的连续子串填入 source_quote 作为点睛之笔；没有合适的句子时可留空。
7. 概念分类、阶段、程度、频率、范围和机制只能来自已裁判证据中的教材原文，不得自行补全。
8. 所有正文text不得出现内部unit_id（如v7u_Nxxxxx）。中文正文目标为600至900字；这是写作目标，不是改变证据规则的理由。
9. 当前盲判状态和校验问题只用于风险提示；不得尝试修复盲判或改变答案。
10. 每个选项的analysis必须写成完整的自然语言解析，包含三个要素：（1）教材相关事实是什么（教材类）或题干明确给出什么（题干对照类）,（2）选项的逐字内容是什么,（3）两者之间具体如何对应或冲突。不要写成”教材原文要点为X。题干中的Y与选项Z可按教材要点直接对应，因此该项正确”的模板句，要写出自然的推理过程。

二、教材类选项的analysis写法（有已裁判证据的选项）
必须把教材中的具体事实、题干的具体条件、选项的具体表述三者串联成自然推理链。例如（仅为写法示范，不要照抄）：
“教材指出结构化是将大额交易拆分为多笔略低于报告限额的做法。题干中该账户有'多笔低于报告限额的存款'，完全吻合结构化的定义特征，因此选项D正确。”
“教材描述微结构化是将资金拆分为更小额的存款，通常低于1,000美元，并分布到多个账户以规避对大额现金流动的监控。而题干仅提及'低于报告限额的多笔存款'，未表明金额低于1,000美元或涉及多个账户这一微结构化的关键特征，因此不属于微结构化，选项C不选。”

三、题干对照类选项的analysis写法（原判断类型为stem_contrast，无教材证据）
必须明确指出选项的哪个概念或要素与题干明确描述的事实之间存在何种具体差异，不能只说”不一致”。例如（仅为写法示范，不要照抄）：
“选项涉及'支票轧空'，这是一种利用支票在不同账户间浮动时间的欺诈手法。而题干明确描述的是'低于报告限额的多笔存款'和'几乎全部在外国被取走'，这指向的是拆分资金以规避报告限额的模式，与支票轧空的运作机制完全不同，因此选项B不选。”
...
五、易错提醒的写法
easy_mistake 的目的是预判考生最可能犯的错误并解释为什么那是错的。选择一个最容易被混淆的干扰项，用自然的段落文字说明：（1）干扰项表面上为什么看起来合理，（2）教材或题干的哪些具体事实可以排除它。例如（仅为写法示范，不要照抄）：
"环境犯罪与非法资源贸易容易混淆，但教材仅将伐木和非法采矿等归入环境犯罪这一上游犯罪类别，并未将'非法资源贸易'列为一个独立的类型。考生若仅凭资源被非法交易的表象而选择非法资源贸易，便会忽略教材分类体系的明确界定。"
"易将结构化与微结构化混淆。教材指出微结构化的特征是金额通常低于1,000美元且分布到多个账户，而题干仅提及'低于报告限额的多笔存款'，未达到微结构化的认定条件。遇到分段存款场景时，应先确认题干是否给出了金额阈值和账户数量等关键信息再做判断。"
不得使用以下句式：（1）"教材对选项X的相关要点是...；对选项Y的相关要点是..."这种引用-对比模板，（2）单独的"判断时应逐项核对题干条件，不要扩大教材中的程度或范围"这种万能结尾。应写成一个完整、自然的段落。
...
八、输出严格JSON，不要Markdown
{{{{
  "answer": ["A"],
  "exam_point": {{{{
    "text": "一句话说明本题考查的规则或概念",
    "cited_unit_ids": ["v7u_N000001"]
  }}}},
  "core_analysis": {{{{
    "text": "只使用已提供事实形成完整判断链",
    "cited_unit_ids": ["v7u_N000001"],
    "source_quote": {{{{
      "unit_id": "v7u_N000001",
  ...
  "option_explanations": [
    {{{{
      "option": "A",
      "analysis": "【必填】该选项的完整自然语言解析。教材类选项须把教材具体事实、题干条件、选项内容串成自然推理链；stem_contrast类选项须指出选项与题干的具体差异是什么。禁止使用'教材原文要点为X。题干中的Y与选项Z可按教材要点直接对应'或'选项明确涉及X而题干明确给出Y，两者不一致'这类模板句。每种错误写明具体理由，不要用'题干要素不匹配'这类标签代替分析。",
  ...
  "easy_mistake": {{{{
    "text": "【必填】选择一个最容易被混淆的干扰项，写成完整的自然段落：该干扰项表面上为什么看似合理，教材或题干的哪些具体事实可以排除它。禁止使用'教材对选项X的相关要点是...对选项Y的相关要点是...判断时应逐项核对题干条件'这类模板，也不要写成'判断时只使用题干明确给出的事实'这类泛泛的提醒。",
    "cited_unit_ids": ["v7u_N000001"]
  }}}}
}}}}"""
...
def _normalize_grounded_block(
    ...
    issues = _unsupported_relation_issues(
        block["text"], block["cited_unit_ids"], unit_map
    )
    if issues:
        return {"text": INSUFFICIENT_TEXT, "cited_unit_ids": []}, [
            f"{location}: {issue}" for issue in issues
        ]
    return block, []
...
def _fallback_exam_point(
    ...
    return {
        "text": f"本题考查题干条件与选项{labels}所依据规则之间的对应关系。",
