...
def build_prompt(
    ...
    return f"""你是一位CAMS反洗钱讲师，正在给学生讲解这道题的解析。请根据已锁定答案和题干事实，结合下面提供的参考资料，生成V3.1结构化解析。你不负责重新判题，也不负责重新检索。

一、硬约束
1. 固定答案是“{predicted}”。不得改判，每个选项的正误由固定答案决定。
2. 所有事实只能来自下面提供的参考资料，不得猜测或引入外部知识。不得照搬heading_context中的案例名。
3. 正文不得出现内部标记词汇（如“已裁判证据”“共享框架证据”“补充候选”“heading_context”等）和 unit_id（如v7u_Nxxxxx）。
4. 每个选项的analysis写成自然段落，把材料事实、选项内容和题干条件串成推理，不要写成“材料要点为X，选项为Y，二者对应/不一致”的模板填空。
5. 做排他性判断（如“仅将X列入”“不属于Y”）时需确认材料中有对应的原文；材料未明确给出的排除结论不要写。

二、教材类选项的analysis写法
把参考资料中的事实、选项的具体内容、题干的条件串成自然的推理段落。选项包含具体行为、金额、主体等细节时展开说明；选项只是一个简短概念时直接核对。
注意：
- 不要用“选项写的是”“选项A的逐字内容是”“题干明确给出的是”这类标签句式开头，把信息融进自然句子中。
- 不要用“材料原文要点为X”这种引用前缀，直接陈述事实。
- 结尾不需要“因此该项正确/不选”，选项标签已经说明了判断。

三、题干对照类选项的analysis写法（无教材证据）
指出选项与题干事实之间的具体差异。不要把“题干没提到X”自动等同于“X不存在”——只说题干提供了什么、选项涉及了什么、二者有何差异。

四、考点的写法
告诉考生怎么判断，而非描述考了什么概念。用一句自然的话概括判断方向。例如：
“区分结构化洗钱与微结构化、贸易洗钱等手法的交易特征，依据拆分金额与账户分布进行判断。”
不要写成分步操作指南（“先提取X，再对照Y，最后判断Z”）。
...
六、易错提醒的写法
选一个最容易被混淆的干扰项，用自然段落说明它为什么看似合理、参考资料中的什么事实可以排除它。
...
{{{{
  ...
  "option_explanations": [
    {{{{
      "option": "A",
      "analysis": "自然段落：把参考资料中的事实、选项内容、题干条件串成推理。不要用“选项写的是”“题干明确给出的是”开头，不要用“材料要点为X”做引用前缀，结尾不要加“因此该项正确/不选”。",
