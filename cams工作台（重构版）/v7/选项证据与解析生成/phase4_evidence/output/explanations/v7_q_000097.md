...
- **C 错误（教材直接依据）**：11项即时成果用于评估司法管辖区AML/CFT体系的有效性，并按照低、中等、显著和高进行评级（P150）。它们是所有受评估司法管辖区通用的有效性评估框架，不是专门针对需要加强监测的高风险司法管辖区提出的建议。

## 【易错提醒】

40项建议规定各司法管辖区应建立哪些制度和控制措施，风险为本方法是其基础原则；11项即时成果则用于判断这些制度在实践中是否有效。即时成果是评估工具，不是针对灰名单或黑名单国家的整改建议。
