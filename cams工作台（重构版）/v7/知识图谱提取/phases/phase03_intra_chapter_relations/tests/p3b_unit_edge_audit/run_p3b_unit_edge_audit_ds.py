...
DEFAULT_PROMPT = TEST_DIR / "p3b_unit_edge_support_audit_v1.md"
DEFAULT_P2B_RUN_PREFIX = "p2b_first5_reviewed_20260706_"
DEFAULT_BASE_URL = "https://api.deepseek.com/v1"
API_KEY_ENV_NAMES = ("DEEPSEEK_API_KEY", "DS_API_KEY", "DS_KEY")
ALLOWED_AUDIT_STATUS = {"supported", "weak_support", "unsupported"}
...
def load_p2b_edges(core_point_id: str, p2b_run_prefix: str) -> list[dict[str, Any]]:
...
def build_input(relations_path: Path, p2b_run_prefix: str, limit: int | None = None) -> dict[str, Any]:
    ...
    for rel in relations:
        items.append(
            {
                "p3_relation": rel,
                "source_p2b_edges": load_p2b_edges(str(rel.get("source_core_point_id")), p2b_run_prefix),
                "target_p2b_edges": load_p2b_edges(str(rel.get("target_core_point_id")), p2b_run_prefix),
            }
        )
    return {
...
def build_messages(prompt_text: str, audit_input: dict[str, Any]) -> list[dict[str, str]]:
    return [
        {"role": "system", "content": prompt_text},
        {
            "role": "user",
            "content": "Audit P3A relations using only P2B unit-edge support. Return one JSON object only.\n\n"
            + canonical_json(audit_input),
        },
    ]


def validate_output(audit_input: dict[str, Any], output: dict[str, Any] | None) -> list[dict[str, Any]]:
    issues: list[dict[str, Any]] = []
    if output is None:
        return [{"issue": "model_output_malformed"}]
    expected_ids = {item["p3_relation"]["relation_id"] for item in audit_input.get("items") or []}
    audits = output.get("relation_audits")
    if not isinstance(audits, list):
        return [{"issue": "relation_audits_not_list"}]
    seen: set[str] = set()
    for audit in audits:
        if not isinstance(audit, dict):
            issues.append({"issue": "audit_not_object"})
            continue
        rel_id = str(audit.get("p3_relation_id") or "")
        status = str(audit.get("audit_status") or "")
        if rel_id not in expected_ids:
        ...
        if status not in ALLOWED_AUDIT_STATUS:
            issues.append({"issue": "invalid_audit_status", "p3_relation_id": rel_id, "audit_status": status})
    missing = sorted(expected_ids - seen)
    if missing:
        issues.append({"issue": "missing_audits", "p3_relation_ids": missing})
    return issues
...
def run(args: argparse.Namespace) -> None:
    ...
    audit_input = build_input(args.relations_file.resolve(), args.p2b_run_prefix, args.limit)
    run_dir = (args.run_dir or (TEST_DIR / "runs" / args.run_slug)).resolve()
    messages = build_messages(prompt_text, audit_input)
    write_json(run_dir / "input_p3b_audit.json", audit_input)
    ...
    issues = validate_output(audit_input, parsed)
    manifest = {
        "schema_version": "p3b_unit_edge_audit_test_manifest_v1",
        ...
        "input_sha256": sha256_text(canonical_json(audit_input)),
        "usage": usage,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--run-slug", default="p3b_unit_edge_audit_test")
