...
4. **页码幻觉**：AI引用的所有（PXX）页码是否都在"证据池所有有效页码"列表中？
   逐一核对AI解析中每个页码引用，是否出现在有效页码清单中。不在清单中的即为页码幻觉。
...
def _build_review_prompt(
    ...
    for uid in sorted(unit_map.keys()):
        ...
        pdf = pg.get("pdf_page", "") or unit.get("pdf_page", "")
        printed = pg.get("printed_page", "") or unit.get("printed_page", "")
        page_info = f"PDF第{pdf}页 / 书内第{printed}页" if pdf else ""
        if pdf and str(pdf).strip().isdigit():
            all_pool_pages.add(int(pdf))
