...
_WORKBENCH_DIR = Path(__file__).resolve().parents[2]
...
def _find_new_question_evidence_pool() -> Path:
    candidates = []
    for child in _WORKBENCH_DIR.iterdir():
        path = child / "pipeline" / "evidence_pool.py"
        if path.exists() and child.resolve() != _MODULE_DIR.resolve():
            candidates.append(path)
    if not candidates:
        raise FileNotFoundError("Cannot find new-question pipeline/evidence_pool.py beside this module")
    return candidates[0]
