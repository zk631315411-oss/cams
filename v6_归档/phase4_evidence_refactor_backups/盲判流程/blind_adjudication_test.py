...
def load_index(pkl_path: str | Path) -> dict[str, Any]:
    ...
    with open(pkl_path, "rb") as f:
        idx = pickle.load(f)
    # Merge printed_page from KG graph into unit_lookup (index doesn't carry it)
    if KG_GRAPH_PATH.exists():
        with open(KG_GRAPH_PATH, "r", encoding="utf-8") as f:
            kg = json.load(f)
        for unit in kg.get("units", []) or []:
            uid = str(unit.get("unit_id", "")).strip()
            if uid and uid in idx.get("unit_lookup", {}):
                idx["unit_lookup"][uid]["printed_page"] = unit.get("printed_page")
        print(f"[load] printed_page 已从KG注入 unit_lookup")
    print(
