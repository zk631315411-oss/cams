...
Do not classify a card as `assessment` merely because it contains words like `review`, `assess`, or `determine`. If the primary objective is operational handling, such as generating alerts, reviewing alerts, closing or escalating alerts, filing reports, conducting screening, verifying identity, or completing a monitoring/control workflow, use `execution`.
...
```text
Transaction monitoring systems generate alerts, staff review alerts, and the workflow outputs explained or potentially suspicious activity.
This is execution, not assessment, because the card represents operational alert handling.
```
