...
- **A 错误（媒介不匹配）**：Vishing的攻击媒介为电话或VoIP语音通话。题干明确攻击媒介为「电子邮件」，媒介层面直接排除。
- **B 错误（攻击路径不匹配）**：Pharming通过DNS缓存投毒或hosts篡改，将用户静默重定向至假冒网站——用户全程不会「收到邮件并打开附件」。攻击路径与题干不符。
- **C 错误（媒介不匹配）**：SMiShing的攻击媒介为SMS短信。题干攻击媒介为电子邮件，渠道不匹配。

## 【易错提醒】

四种网络钓鱼亚型的区分维度是**攻击媒介**——题干「电子邮件」一词直接排除A（电话）和C（短信），「收到欺诈邮件」的主动诱导路径排除B（DNS静默重定向）。锁定媒介后，再判断是否「定向冒充特定机构」即可确认spear phishing。
