...
> 版本边界：本文保存 V6 到 V7 内容生产链的阶段性技术路线。涉及旧前端发布、91 题复核、V6 输入路径或 `tools/v7_release/` 的内容均为历史记录，不是当前正式运行说明。当前边界见 `项目交接/正式资产与SOP索引.md`。
...
历史首次扫描曾标记 91 题需复核；该数字早于后续终审，不得作为当前 395 题的验收统计。
