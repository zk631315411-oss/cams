...
DEFAULT_PROMPT = PHASE_DIR / "prompts" / "p3b_relation_evidence_binding_v1.md"
DEFAULT_P2A_RUN_PREFIX = ""
DEFAULT_P2B_RUN_PREFIX = ""
...
def resolve_p2a_run_dir(section_id: str, p2a_run_prefix: str) -> Path | None:
    if p2a_run_prefix:
        candidate = P2_DIR / "runs" / f"{p2a_run_prefix}{section_id}"
...
def resolve_p2b_run_dir(core_point_id: str, p2b_run_prefix: str) -> Path | None:
    if p2b_run_prefix:
        candidate = P2_DIR / "runs" / f"{p2b_run_prefix}{core_point_id}"
        if (candidate / "parsed_response.json").exists():
            return candidate

    matches = sorted(
        [path for path in (P2_DIR / "runs").glob(f"p2b*{core_point_id}") if (path / "parsed_response.json").exists()],
        key=lambda path: path.stat().st_mtime,
        reverse=True,
    )
    return matches[0] if matches else None
...
def load_unit_texts(section_id: str, p2a_run_prefix: str) -> dict[str, str]:
    run_dir = resolve_p2a_run_dir(section_id, p2a_run_prefix)
    if run_dir is None:
        return {}
...
def load_p2b_edges(core_point_id: str, p2b_run_prefix: str, p2a_run_prefix: str) -> list[dict[str, Any]]:
    run_dir = resolve_p2b_run_dir(core_point_id, p2b_run_prefix)
    if run_dir is None:
        return []
...
def load_cp_title(core_point_id: str, p2a_run_prefix: str) -> str | None:
    ...
    run_dir = resolve_p2a_run_dir(section_id, p2a_run_prefix)
    if run_dir and (run_dir / "parsed_response.json").exists():
        candidates.extend(read_json(run_dir / "parsed_response.json").get("core_points") or [])
...
def build_input(relations_path: Path, p2a_run_prefix: str, p2b_run_prefix: str, relation_ids: set[str] | None, limit: int | None) -> dict[str, Any]:
    ...
    return {
        "binding_batch_id": "p3b_relation_evidence_binding",
...
def run(args: argparse.Namespace) -> None:
    ...
    manifest = {
        "schema_version": "p3b_relation_evidence_binding_manifest_v1",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--p2a-run-prefix", default=DEFAULT_P2A_RUN_PREFIX, help="Optional legacy P2A run prefix. Empty default auto-discovers reviewed P2A runs.")
    parser.add_argument("--p2b-run-prefix", default=DEFAULT_P2B_RUN_PREFIX, help="Optional legacy P2B run prefix. Empty default auto-discovers P2B runs by core_point_id.")
    parser.add_argument("--relation-id", action="append", default=[])
    parser.add_argument("--limit", type=int, default=None)
    parser.add_argument("--run-slug", default="p3b_relation_evidence_binding")
