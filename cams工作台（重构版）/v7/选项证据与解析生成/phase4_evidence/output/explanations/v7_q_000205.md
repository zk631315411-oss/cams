...
- **B 错误**：教材把传票、法院命令列为最正式的请求，并说明其规定记录提交期限（P361）。B项只说「未获传票」即提供文件和证词，也未给出法院命令、生产令等其他正式依据，不能建立题干所要求的强制回应义务。
- **C 错误**：教材说明执法请求虽可采用不同书面形式，但法院命令和传票具有最正式的地位（P361）。C项只有一般「书面请求」，没有说明授权获取特权文件的法院命令或其他正式法律依据，其合法性与强制回应义务均不如D项明确。
