...
_CACHE_DIR = _HERE / "outputs" / "cache"
...
def load_new_question_runtime(
    ...
    def _read_kg_json(filename: str) -> list[Any] | dict[str, Any]:
        path = run_step1.KG_DIR / filename
        if not path.exists():
            print(f"[evidence_pool] KG file not found, skipping: {filename}")
            return []
        return run_step1.read_json(path)

    sections = _read_kg_json("sections.json")
    edges = _read_kg_json("edges.json")
    cs_map = _read_kg_json("card_section_map.json")
    ...
    relations_path = run_step1.DATA / "derived" / "card_relations.json"
