# p2-ch7-h3 题库软件版解析预览

可导出题目数：13

教材章节：未映射

题型：single

题干：以下哪一项描述了金融情报机构(FIU)在进行跨境洗钱调查时正式的信息共享与合作方式？

英文题干：Which of the following describes a formal method of information sharing and cooperation between Financial Intelligence Units (FIUs) when conducting cross-border money laundering investigations?

选项：

- A. 金融行动特别工作组(FATF)、欧洲委员会打击洗钱组织(MONEYVAL)及其他作为金融情报机构联络点的区域机构
  English: FATF, MONEYVAL, and other regional bodies acting as a contact point for FIUs
- B. 狼堡集团作为金融情报机构的沟通平台
  English: The Wolfsberg Group serving as a communication platform for FIUs
- C. 反洗钱金融情报机构之间为建立结构化信息共享而签订的谅解备忘录(MOU)
  English: Memoranda of Understanding (MOUs) between FIUs establishing structured information sharing
- D. 互助法律援助条约(MLATS),允许金融情报机构请求逮捕和获取证据
  English: Mutual Legal Assistance Treaties (MLATs) allowing FIUs to request arrests and evidence

## 【AI答案】

C

> **需人工复核**
>
> - 盲判校验未通过

## 【考点】

区分FIU正式合作方式与标准制定机构、司法协助的功能边界

## 【核心解析】

谅解备忘录（MOU）是一种补充司法协助条约（MLAT）的正式工具，用于简化程序并为联合调查确立协议（P233）。FIU之间的跨境合作通常建立在双边或多边MOU基础上，以形成结构化的情报共享框架。题干询问FIU在跨境洗钱调查中正式的信息共享与合作方式，C选项描述的正是FIU通过签订MOU建立结构化信息共享，这直接对应教材中MOU作为正式合作补充机制的角色。相比之下，其他选项或是制定标准的行业组织，或是服务于司法当局的法律条约，在主体和功能上与题干要求的“FIU之间的正式合作方式”匹配度不如C。

教材原句："MLATs may be supplemented by memoranda of understanding to streamline procedures and outline protocols for joint investigations."

## 【错误项分析】

- **A 错误（教材直接依据）｜主体或阶段错配**：FATF和MONEYVAL等区域机构的主要职能是制定反洗钱标准并协助实施 FATF 40 项建议（P141），它们作为联络点并非FIU之间进行信息共享的正式方式。题干问的是FIU间的直接合作机制，这些机构更偏向标准推动者而非合作工具。
- **B 错误（教材直接依据）｜主体或阶段错配**：沃尔夫斯堡集团是由全球银行组成的协会，制定金融犯罪风险管理政策与指引（P168），并非FIU的沟通平台。题干的核心是FIU间的正式合作方式，狼堡集团的成员主体是银行，功能是发布指引，与FIU间的信息共享机制无直接关系。
- **D 错误（教材定义应用）｜主体或阶段错配**：司法协助条约（MLAT）为传递可用于起诉的证据提供法律依据，其请求通常由法院或检察官提出（P233），因此也被称为司法合作。题干要求的是FIU之间的信息共享方式，而MLAT的主体是司法当局，不适用于FIU直接请求逮捕，其功能和主体与题干匹配度低于MOU。

## 【易错提醒】

MLAT是为法院／检察官设计的司法协助条约（P233），用于传递起诉证据，不用于FIU自身的信息交换。而MOU是由合作各方（包括FIU）签订的简化程序协议，直接支持结构化信息共享。

---

教材章节：未映射

题型：single

题干：在两个国家之间签署并规范跨境信息共享的法律文书被称为:

英文题干：A legal instrument that is executed between two nations and governs cross-border information sharing is known as a:

选项：

- A. 司法互助条约
  English: Mutual legal assistance treaty
- B. 意向书
  English: Memorandum of understanding
- C. 紧急信息请求
  English: Request for urgent information
- D. 协议备忘录
  English: Declaration of understanding

## 【AI答案】

A

## 【考点】

识别MLAT（司法互助条约）的核心定义与功能

## 【核心解析】

教材将“司法协助”（Mutual legal assistance）定义为司法管辖区之间为获取执法机构无法直接共享的信息和证据而建立的公约与协定框架（P233）。在这个框架下，“司法协助条约”（MLAT）是具体的法律文书，为传递可用于起诉和司法程序的证据提供法律依据（P233）。题干中“两个国家之间签署”对应教材中的“between jurisdictions”，“规范跨境信息共享”对应“obtaining information and evidence”，“法律文书”（legal instrument）对应“treaties”和“legal basis”。三个关键要素——双边国家、跨境信息共享、法律文书——与MLAT的定义完全吻合。

教材原句："Mutual legal assistance treaties (MLATs) provide a legal basis for transmitting evidence that can be used for prosecution and judicial proceedings."

## 【错误项分析】

- **B 错误（教材直接依据）｜概念混淆**：教材提到谅解备忘录可“补充”司法协助条约以简化程序（P233），说明它在跨境合作中处于辅助地位，其法律约束力和正式程度均不如MLAT。题干要求的是一项直接规范跨境信息共享的“法律文书”，MLAT的匹配度更高。
- **C 错误（题干对照）｜题干要素不匹配**：教材中未将“紧急信息请求”定位为国家间签署的法律文书。教材涉及“紧急”的上下文是FIU对恐怖融资等紧迫事项做有限检查以尽快传播信息（P231），或执法机构在紧急情况下签发搜查令（P361），均不是两国间签署的法律框架。
- **D 错误（题干对照）｜题干要素不匹配**：英文原选项为“Declaration of understanding”，与教材中提到的“memoranda of understanding”（谅解备忘录）不同。教材未对“Declaration of understanding”做定义，且该表述在跨境信息共享语境下缺乏作为两国间正式法律文书的教材支撑。

## 【易错提醒】

MLAT（司法协助条约）与MOU（谅解备忘录）易混淆。教材给出的区分标准是：MLAT为传递证据提供法律依据（P233），是正式的法律文书；MOU是“补充”MLAT以简化程序和概述联合调查协议的辅助工具（P233），法律约束力不如MLAT强。

---

教材章节：未映射

题型：single

题干：A国的反腐败机构一直在调查可能的贿赂和贪污所得向B国转移的情况,目前正准备将此案提交当地法院审理.哪种获取起诉该案所需证据的方法最为有效？

英文题干：Country A's anti-corruption authority has been investigating the movement of potential proceeds of bribery and corruption to Country B and is now preparing to take the case to a local court. Which method of obtaining the required evidence to prosecute the case would be most effective?

选项：

- A. 直接由A国反腐败机构负责人向B国相应执法机构提出正式请求
  English: Make an official request directly from the head of Country A's anti-corruption authority to Country B's corresponding law enforcement authority
- B. 依据A国与B国之间的司法互助条约,提出获取所需证据的请求
  English: Make a request for the required evidence using the mutual legal assistance treaty between Country A and Country B
- C. 利用国际刑事警察组织(国际刑警组织)的网络从B国获取所需的证据
  English: Use the International Criminal Police Organization (INTERPOL) network to obtain the required evidence from Country B
- D. 通过埃格蒙特安全网络从B国金融情报机构获取所需证据
  English: Obtain the required evidence from Country B's Financial Intelligence Unit through the Egmont Secure Web

## 【AI答案】

B

## 【考点】

跨境起诉取证应优先选择司法互助条约（MLAT）而非其他合作渠道

## 【核心解析】

司法协助条约（MLAT）是司法管辖区之间为获取信息和证据而建立的公约和协议框架，为传递可用于起诉和司法程序的证据提供法律依据（P233）。题干中A国反腐败机构已调查完毕贿赂和贪污所得向B国转移的案件，正“准备将此案提交当地法院审理”，此时需要的是能在法庭上使用的正式证据。MLAT恰好为这一需求提供了法定的跨境证据传递通道——通过司法协助请求（MLAR），A国法院或检察机关可以从B国获取具有法律效力的证据，直接用于起诉。其他渠道虽然也能实现信息交换，但只有MLAT框架下的证据传递专门为“起诉和司法程序”设计，与题干场景最匹配。

教材原句："Mutual legal assistance treaties (MLATs) provide a legal basis for transmitting evidence that can be used for prosecution and judicial proceedings."

## 【错误项分析】

- **A 错误（题干对照）｜范围或程度偏差**：反腐败机构负责人直接向B国执法机构发正式请求，缺乏MLAT那样的法律框架支撑。教材提到执法部门的信息请求需要书面程序（P361），但这类机构间直接对接通常用于调查阶段的信息交换，而非为起诉准备法庭可采纳的证据。题干已进入“准备提交法院审理”阶段，MLAT比直接机构间请求更能确保证据的司法可用性。
- **C 错误（题干对照）｜概念混淆**：国际刑警组织（INTERPOL）的主要功能是支持警务合作和信息交换，教材在Eurojust/Europol的案例（P232）中展示了这类机构在调查阶段的协调角色，但并未将其定位为向法院提供起诉证据的渠道。题干要求的是“获取起诉该案所需证据”，MLAT更直接服务于起诉目的，而国际刑警组织的网络更侧重于调查支持。
- **D 错误（教材直接依据）｜概念混淆**：埃格蒙特集团是各国金融情报机构（FIU）的合作网络，主要促进情报共享以打击洗钱和恐怖融资（P167）。FIU共享的是金融情报，用于分析和线索发现，不是直接为法庭起诉准备证据。题干需求是“起诉该案所需证据”，MLAT专门为起诉提供法律依据，而埃格蒙特安全网络偏向情报层面的合作，在诉讼证据获取方面不如MLAT直接。

## 【易错提醒】

金融情报机构（FIU）通过埃格蒙特网络共享的是“情报”，用于分析和调查；而MLAT传递的是可用于法庭的“证据”，两者功能定位不同——情报帮助发现线索，证据支持定罪。看到题干有“起诉”字眼时，优先考虑MLAT而非FIU渠道。

---

教材章节：未映射

题型：single

题干：为协助调查一起跨境洗钱案件，作为埃格蒙特集团成员的金融情报机构(FIU)可以:

英文题干：To provide aid in investigating a cross-border money laundering case, a Financial Intelligence Unit (FIU) that is a member of the Egmont Group can:

选项：

- A. 直接与另一国的金融机构联系并分享与调查相关的信息
  English: Directly contact financial institutions in another country and share information pertinent to the investigation
- B. 直接与其他国家的金融情报机构(FIU)联系并分享与调查相关的信息
  English: Directly contact other FIUs in another country and share information pertinent to the investigation
- C. 授权其执法调查人员协助另一国正在进行的重大调查
  English: Deputize its law enforcement investigators to assist in a material ongoing investigation in another country
- D. 协助另一国执法部门进行重大在办案件的调查
  English: Assist law enforcement in another country with a material ongoing investigation

## 【AI答案】

B

> **需人工复核**
>
> - 答案冲突：解析{'B'} vs 题库{'D'}

## 【考点】

埃格蒙特集团框架下FIU的核心职能：FIU对FIU的情报交换

## 【核心解析】

埃格蒙特集团是各国金融情报机构（FIU）的国际网络，旨在促进成员间的合作与情报共享以打击洗钱等金融犯罪（P167）。根据FATF标准和埃格蒙特集团原则，FIU应自发或应请求相互传播金融情报（P231），且FIU可与其他国家的FIU建立联系（P231）。题干问“作为埃格蒙特集团成员的FIU”在协助跨境洗钱调查时能做什么——关键词是“埃格蒙特集团成员”和“FIU”。教材框架下埃格蒙特集团直接连接的是各国FIU，而非金融机构或执法部门。因此FIU通过埃格蒙特网络直接与另一国FIU联系并分享情报是集团设计的最核心、最直接的协作方式。选项B的“直接联系其他国家的FIU”完整吻合这一框架。

教材原句："Under FATF standards and principles set by the Egmont Group of FIUs, FIUs are expected to disseminate financial intelligence to each other, either spontaneously or on request."

## 【错误项分析】

- **A 错误（教材直接依据）｜主体或阶段错配**：埃格蒙特集团的网络连接的是各国FIU而非金融机构（P167）。题干场景下，FIU更应通过对方国家的FIU间接获取信息，而非直接联系另一国金融机构，后者超出了埃格蒙特框架下FIU之间情报交换的典型路径。
- **C 错误（题干对照）｜主体或阶段错配**：某些辖区FIU可同时承担执法机构角色（P229），但题目问的是“作为埃格蒙特集团成员的FIU”能做什么——集团的职能聚焦于FIU之间的情报交换与协作，授权执法调查人员跨境行动更属于司法互助或执法合作的范畴，不如B直接匹配埃格蒙特的核心使命。
- **D 错误（教材直接依据）｜主体或阶段错配**：FIU向执法部门传播情报是其国内职能（P231），但直接协助另一国执法部门进行案件调查并非埃格蒙特集团框架下FIU的典型跨境职能。教材描述的跨境合作路径是FIU→FIU→执法部门，而非FIU直接跳至另一国执法机构，选项B的FIU对FIU路径更吻合。

## 【易错提醒】

埃格蒙特集团搭建的是FIU对FIU的合作网络（P167），信息在FIU之间流通后再由各自FIU向国内执法部门传播（P231）。跨境场景下的协作链路是“FIU→外国FIU→外国执法部门”，而非FIU直接对外国执法部门或金融机构行动。

---

教材章节：未映射

题型：single

题干：该国金融情报机构(FIU)收到一份可疑活动报告(SAR),其中涉及大量可疑资金转移,不仅在其本国境内,还涉及外国.为确定是否需将此事移交本国检察机关处理,还需从外国获取更多信息.在该情景中,以下哪个陈述是正确的？

英文题干：The Financial Intelligence Unit (FIU) in a country has received a suspicious activity report (SAR) which involves significant suspicious fund transfers, not only within its jurisdiction but also in a foreign country. Further information is required from the foreign country to determine whether the matter needs to be referred for prosecution locally. Which of the following statements is true in this scenario?

选项：

- A. 从外国获取信息违反了国际数据保护法.
  English: It is against international laws on data protection to access information from foreign countries
- B. 国家主权意味着信息不能从外国获取
  English: Sovereignty of nations means that information cannot be accessed from foreign countries
- C. 任何与洗钱有关的信息,均可随时从任何组织接收,不受司法管辖区的限制.
  English: Any information related to money laundering can be received at any time from any organization, regardless of jurisdiction
- D. 埃格蒙特集团的成员国之间可以相互请求信息协助.
  English: Egmont Group members can request information assistance from each other

## 【AI答案】

D

## 【考点】

埃格蒙特集团框架下FIU跨境信息共享机制

## 【核心解析】

埃格蒙特集团是各国金融情报机构（FIU）的国际网络，旨在促进成员间合作与情报共享以打击洗钱等金融犯罪（P167）。根据FATF标准和埃格蒙特集团原则，FIU应自发或应请求相互传播金融情报（P231）。题干中，FIU收到涉及本辖区及外国的可疑资金转移报告，需要从外国获取更多信息以决定是否移交起诉——这正是埃格蒙特集团成员间信息协助机制的典型应用场景。因此，埃格蒙特集团成员国之间可以相互请求信息协助，直接匹配题干所需。

教材原句："Under FATF standards and principles set by the Egmont Group of FIUs, FIUs are expected to disseminate financial intelligence to each other, either spontaneously or on request."

## 【错误项分析】

- **A 错误（题干对照）｜范围或程度偏差**：选项A将跨境获取信息绝对化为“违反国际数据保护法”，但教材仅指出数据保护法律可能对信息共享构成挑战（P194），而非绝对禁止。在埃格蒙特集团框架下，成员间的信息共享本身是受认可的机制，题干场景更直接匹配的是合法合作渠道，而非违法风险。
- **B 错误（题干对照）｜概念混淆**：选项B以“国家主权”为由否定从外国获取信息的可能，但教材明确记载FIU可以访问其他国家的FIU（P231），且打击洗钱需要国内与国际合作（P233）。国家主权并不排斥在既定合作框架内共享情报，题干描述的FIU间信息请求正是这种合作框架的体现。
- **C 错误（题干对照）｜范围或程度偏差**：选项C说“不受司法管辖区的限制”“随时从任何组织接收”，表述过于绝对。教材记载FIU信息共享是在FATF标准和埃格蒙特集团原则的框架内进行的（P231），并非无边界无限制。题干需要的是有据可循的协助渠道，而非这种笼统的绝对陈述。

## 【易错提醒】

（无）

---

教材章节：未映射

题型：single

题干：以下哪项是金融情报机构(FIU)的核心职能,如金融行动特别工作组(FATF)第29号建议所述？

英文题干：Which of the below is a core function of a financial intelligence unit (FIU), as described in FATF Recommendation 29?

选项：

- A. 资助先进科技监控工具的研发，以供全国使用
  English: Sponsoring the research and development of advanced technological surveillance tools to be used nationally
- B. 起诉属于国家管辖范围内的重大洗钱或恐怖融资案件
  English: Prosecuting significant cases of money laundering or terrorist financing falling under national jurisdiction
- C. 与私营部门共享从国家执法机构收集到的实时犯罪情报
  English: Sharing real-time criminal intelligence gathered from national law enforcement agencies with the private sector
- D. 充当全国可疑活动收集与分析中心
  English: Serving as a national center for the collection and analysis of suspicious activity

## 【AI答案】

D

## 【考点】

识别金融情报机构的三大核心职能：接收、分析、传播

## 【核心解析】

FATF要求各辖区设立金融情报机构（FIU），其核心职能是接收、分析和传播金融情报（P231，P228）。换句话说，FIU就是一个全国性的“信息收发与处理中枢”：各金融机构把可疑交易报告提交给它（接收），它负责把这些零散的信息转化成有价值的情报（分析），再分发给执法部门去用（传播）。选项D“充当全国可疑活动收集与分析中心”正是对这个中枢角色的直接描述——“收集”对应接收，“分析”就是核心工作，“中心”点明了其作为国家节点的地位。其他选项所述职责在教材框架下归属于执法部门或金融机构自身，与FIU的法定核心职能不直接匹配。

教材原句："FATF requires that jurisdictions have FIUs to receive, analyze, and disseminate financial intelligence."

## 【错误项分析】

- **A 错误（教材定义应用）｜主体或阶段错配**：题干问的是FIU被FATF赋予的“核心职能”。教材提到金融机构应使用先进工具监控可疑交易（P65），但这属于机构自身的合规义务。为全国“研发”监控工具与FIU“接收、分析、传播”情报的核心定位相比，匹配度明显更远。
- **B 错误（教材定义应用）｜主体或阶段错配**：“起诉”是执法或检察机关的职责。教材指出公安部调查和起诉洗钱犯罪（P200），而FIU的职能是“分析可疑交易报告”并“支持”执法调查（P149），二者是支持与被支持的关系。题干更直接地指向情报处理职能而非司法起诉职能。
- **C 错误（教材定义应用）｜概念混淆**：教材描述了公私合作伙伴关系可用于“共享信息以支持执法调查”（P237），方向通常是由公共部门（含FIU）与私营部门共享以打击犯罪。题干强调FIU的核心是收集并分析来自金融机构的可疑报告，而非从执法机构收集情报再反过来“与私营部门共享”。

## 【易错提醒】

FIU是情报处理中心，不负责直接的执法行动（如起诉）或为全行业研发工具。它的核心动作链条始终是“接收→分析→分发”，分清情报提供者和执法实施者的角色就能避免选错。

---

教材章节：未映射

题型：single

题干：以下哪个项目最有可能被视为监管机构有效监管制度的一部分？

英文题干：Which of the following programs would most likely be considered part of an effective supervision regime for a regulator?

选项：

- A. 现场和非现场审查、开展国际合作，并可获得一系列制裁和外联资源
  English: Onsite and offsite reviews, cooperating on an international basis, with access to a range of sanctions and outreach
- B. 现场和非现场审查、开展国际和国内合作，并可使用一系列制裁和外联手段
  English: Onsite and offsite reviews, cooperating on an international and domestic basis, with access to a range of sanctions and outreach
- C. 非现场审查、开展国内合作，并可使用一系列制裁和外联手段
  English: Offsite reviews, cooperating on a domestic basis, with access to a range of sanctions and outreach
- D. 现场审查、开展国际和国内合作，但没有外联，也不能使用一系列制裁手段
  English: Onsite reviews, cooperating on an international and domestic basis, without outreach or access to a range of sanctions

## 【AI答案】

B

## 【考点】

有效监管制度的合作范围与工具组合完整性

## 【核心解析】

监管机构有效监管制度的核心在于监管工具是否同时覆盖境内与跨境两个维度。教材明确写道，监管机构在一个司法管辖区内以及国际层面均需开展合作（P231）。此外，监管机构还需拥有确保合规的多种工具，包括处以罚款和针对严重案件的执法行动（P228）。题干问的是监管机构的“有效监管制度”构成，选项B同时具备了现场/非现场两种审查方式、国际与国内双维合作，以及制裁和外联手段，在四组选项中覆盖的要素最完整，符合教材对有效监管制度的描述。选项A缺少国内合作这一维度，选项C缺少现场审查且仅限国内合作，选项D缺少非现场审查、外联手段及制裁工具，相比之下匹配度均不如B。

教材原句："Regulators cooperate both within a jurisdiction and internationally."

## 【错误项分析】

- **A 错误（教材定义应用）｜范围或程度偏差**：教材强调监管机构在一个司法管辖区内和国际层面都要开展合作（P231），A项仅提及国际合作而缺少国内合作这一维度，相比B项对教材合作范围的覆盖不够完整。
- **C 错误（教材定义应用）｜范围或程度偏差**：现场与非现场审查均为监管机构的基础审查手段，C项仅含非现场审查，且合作范围限于国内，缺少教材要求的国际合作维度（P231），要素完整性明显不足。
- **D 错误（教材定义应用）｜范围或程度偏差**：监管机构需要拥有多种合规工具，包括罚款和执法行动等制裁手段（P228），外联工作同样是监管沟通的重要组成（P255）。D项明确缺少制裁手段、外联以及非现场审查三种核心要素，是所有选项中匹配度最低的。

## 【易错提醒】

（无）

---

教材章节：未映射

题型：single

题干：在处理跨境可疑或异常金融活动调查时，哪些监管机构会进行合作？

英文题干：Which regulatory bodies cooperate when dealing with cross-border suspicious or unusual financial activity investigations?

选项：

- A. 欧洲委员会
  English: European Commission
- B. 金融行动特别工作组
  English: Financial Action Task Force
- C. 沃尔夫斯堡集团
  English: Wolfsberg Group
- D. 金融情报机构
  English: Financial Intelligence Units

## 【AI答案】

D

## 【考点】

区分国际反洗钱机构的职能性质（标准制定 vs 操作协作）

## 【核心解析】

跨境可疑金融活动调查中的国际合作，核心执行者是各国金融情报机构（FIU）。根据FATF建议，金融机构有义务向本国FIU报告可疑交易（P148）。在跨境场景下，各国FIU通过埃格蒙特集团等渠道直接交换情报——埃格蒙特集团的核心职能就是信息共享，使FIU能够共享可疑金融活动的情报（P167）。教材进一步明确，这些机构应与国际同行共享信息以处理跨境洗钱和恐怖融资（P229）。以美国为例，FinCEN（即美国FIU）与外国FIU协调打击跨境金融犯罪（P184）。题干限定场景是“处理跨境可疑金融活动调查”——这是FIU的日常操作职能，而非标准制定或行业指引职能。

教材原句："These agencies should also share information with their international counterparts to handle crossborder money laundering and terrorist financing."

## 【错误项分析】

- **A 错误（题干对照）｜主体或阶段错配**：教材中出现“European Commission”（欧盟委员会）时，其角色是制定监管框架（如MiCA，P192），而非直接处理可疑交易调查的合作。欧盟作为司法管辖区标签出现（P298），但教材未赋予其跨境可疑活动调查中的执行协作职能。题干问的是“调查”时的合作机构，欧洲委员会更接近立法与政策制定者，与FIU的实操角色匹配度不同。
- **B 错误（教材直接依据）｜主体或阶段错配**：FATF的核心职能是制定全球反洗钱标准（40项建议）和协调各国打击洗钱的努力（P144），其建议要求各辖区通过信息交换促进国际合作（P149）。但FATF本身是标准制定机构，并非跨境可疑交易调查的实际执行者。题干场景是“调查”合作，执行调查协作的是各国FIU而非FATF这个标准制定平台。当题目问“谁制定反洗钱国际标准”时，FATF才是直接匹配项。
- **C 错误（教材直接依据）｜主体或阶段错配**：沃尔夫斯堡集团是全球银行的行业协会，发布金融犯罪风险管理指引（P168）。教材明确指出其“无执法权”（P168），出版物是为金融机构提供可调整的指导。题干场景是监管机构之间的调查合作，而非行业自律指引的制定。沃尔夫斯堡集团既不是监管机构，也不参与跨境调查协作。

## 【易错提醒】

FATF制定“标准”，FIU执行“操作”。FATF建议各国建立FIU并赋予其跨境情报交换职能，但FATF本身不接收、不分析、不交换可疑交易报告。埃格蒙特集团是全球FIU之间的协作网络，其核心功能是信息共享（P167）。遇到涉及“接收可疑报告”“跨境信息交换”“调查协作”的题干，要找FIU而非FATF。

---

教材章节：未映射

题型：single

题干：A国发布的新闻报道称，一名政治公众人物（PEP）从一家总部位于发展中国家的跨国公司收受了约150万美元的贿赂，这笔钱存入了位于B国的一家金融机构（FI）的账户。两国均为埃格蒙特集团成员。该账户属于该政治公众人物的直系亲属。为核实事实，A国金融情报机构（FIU）通过安全通信渠道向B国对口金融情报机构（FIU）发出正式请求，要求提供进一步信息。根据埃格蒙特原则，B国金融情报机构可以：

英文题干：News published in Country A reports that a politically exposed person (PEP) had received a bribe from a transnational company headquartered in a developing country of approximately US$1.5 million, deposited into an account at a financial institution (FI) located in Country B. Both countries are members of the Egmont Group. The account where the money was deposited belongs to the PEP's immediate family member. To corroborate the facts, the FIU of Country A sent a formal request via secure communication channels for further information from its counterpart FIU in Country B. According to Egmont principles, the FIU of Country B can:

选项：

- A. 提供其掌握的信息，因为该交换发生在两个均为埃格蒙特集团成员的金融情报机构之间，且正式请求是通过安全通信渠道提出的
  English: Provide the information available to them because the exchange is between two FIUs that are members of the Egmont Group, and the formal request was made using secure communication channels
- B. 仅在A国与B国签署谅解备忘录（MoU）的情况下提供所要求的信息
  English: Provide the requested information only if Country A has signed a memorandum of understanding (MoU) with Country B
- C. 指示A国金融情报机构向开设账户的金融机构进一步了解情况
  English: Instruct Country A's FIU to inquire further with the financial institution where the account is held
- D. 如果B国金融情报机构未收到开设账户的金融机构提交的可疑活动报告（SAR），则拒绝该请求
  English: Refuse the request if Country B's FIU has not received a suspicious activity report (SAR) from the financial institution where the account is held

## 【AI答案】

A

## 【考点】

埃格蒙特集团框架下金融情报机构间的情报共享原则

## 【核心解析】

埃格蒙特集团是各国金融情报机构（FIU）的国际网络，旨在促进成员间合作与情报共享以打击洗钱、恐怖融资及其他金融犯罪（P167）。根据FATF标准和埃格蒙特集团原则，FIU应自发或应请求相互传播金融情报（P231）。题干中，A国FIU通过安全通信渠道向同为埃格蒙特成员的B国FIU发出正式请求，这两个条件——同为埃格蒙特成员、通过安全渠道提出正式请求——直接匹配教材所述的情报共享框架，因此B国FIU可以提供其掌握的信息。

教材原句："Under FATF standards and principles set by the Egmont Group of FIUs, FIUs are expected to disseminate financial intelligence to each other, either spontaneously or on request."

## 【错误项分析】

- **B 错误（教材直接依据）｜概念混淆**：谅解备忘录（MoU）可以补充和简化跨境合作程序（P233，P236），但埃格蒙特集团已通过其治理文件标准化了成员间的信息交换（P167-P168），MoU并非成员间情报共享的前提条件。A项所依据的“埃格蒙特成员+安全渠道”框架对本题场景的匹配更直接。
- **C 错误（题干对照）｜主体或阶段错配**：金融机构确有义务响应FIU的信息请求（P362），但这描述的是FIU与本国金融机构之间的关系。题干场景是两国FIU之间的情报交换，B国FIU的角色是“提供信息”而非指示对方FIU代行职权，该项不符合埃格蒙特集团下的FIU间合作模式。
- **D 错误（教材直接依据）｜证据不足**：FIU的信息来源包括报告实体、国内来源及其他国家FIU（P231），其情报传播不依赖于本国金融机构是否已就该事项提交SAR。将未收到SAR作为拒绝境外FIU请求的理由，与埃格蒙特集团促进信息共享的原则不符。

## 【易错提醒】

（无）

---

教材章节：未映射

题型：single

题干：根据埃格蒙特集团原则，金融情报机构（FIU）之间的信息交换应遵循以下原则：

英文题干：Under the Egmont Group Principles, information exchange among financial intelligence units (FIUs) should be conducted:

选项：

- A. 不期望在信息将如何使用方面获得互惠
  English: Without the expectation of reciprocity on how the information will be used
- B. 仅当外国金融情报机构的地位与执法相关时
  English: Only if the status of the foreign FIU is related to law enforcement
- C. 自由地、自发地，并应请求，在互惠的基础上
  English: Freely, spontaneously, and upon request, on the basis of reciprocity
- D. 对所提供的财务和行政信息数量设定限制
  English: With set limits on the amount of financial and administrative information provided

## 【AI答案】

C

## 【考点】

埃格蒙特原则下FIU信息交换的"自发、应请求、互惠"三要素

## 【核心解析】

埃格蒙特集团是各国金融情报机构（FIU）的国际网络，旨在促进成员间合作与情报共享（P167）。根据FATF标准及埃格蒙特集团原则，FIU应**自发地或应请求**（spontaneously or on request）相互传播金融情报（P231）。

将四个选项逐一对照教材给出的这一句原文：选项C所述"自发地，并应请求"与P231的措辞直接吻合——spontaneously 对应自发分享，upon request 对应应请求提供。教材虽未在P231处展开论述"互惠"与"自由地"这两个措辞，但"FIU应相互传播"（disseminate to each other）本身即隐含了双向、对等的交换结构——不是单向请求、不是设限提供、也不是以对方地位为前提。相比之下，选项A否定互惠、选项B以执法地位为前置条件、选项D限制信息数量，均与P231"相互传播""自发或应请求"所承载的对等开放精神不一致。因此在教材框架下，C项最吻合P231对FIU信息交换原则的描述。

教材原句："Under FATF standards and principles set by the Egmont Group of FIUs, FIUs are expected to disseminate financial intelligence to each other, either spontaneously or on request."

教材原句："Under FATF standards and principles set by the Egmont Group of FIUs, FIUs are expected to disseminate financial intelligence to each other, either spontaneously or on request."

## 【错误项分析】

- **A 错误（教材定义应用）｜概念混淆**：选项A"不期望在信息将如何使用方面获得互惠"混淆了两个层面：埃格蒙特原则要求信息交换以互惠为基础——FIU之间应双向开放情报共享，而非单方面索取；但提供方FIU对共享信息的具体使用方式保留事先同意权，这不等同于"不期望互惠"。教材描述的FIU信息传播是相互的（P231 "to each other"），而非单向的"不期待回报"。
- **B 错误（教材直接依据）｜范围或程度偏差**：埃格蒙特集团原则未将信息交换限定于对方FIU的地位与执法相关。P167明确埃格蒙特集团的成员是各国FIU，无论其在国内属于行政型、执法型还是司法型，均可参与信息交换与合作，不以执法地位为前置条件。
- **D 错误（教材定义应用）｜概念混淆**：对信息交换"设定数量限制"与埃格蒙特原则的基本精神相悖。P231要求FIU"spontaneously or on request"传播情报，意在鼓励尽可能广泛的情报共享，而非人为设限。

## 【易错提醒】

"互惠/reciprocity"在FIU信息交换语境下指向机构间对等、双向的共享义务，而非交易式的算计回报。A选项正是利用这一歧义制造干扰——将"交换以互惠为基础"偷换为"期待对方报偿"。

---

教材章节：未映射

题型：single

题干：以下哪一项最能描述监管机构、执法机构和金融情报机构(FIU)在洗钱调查(包括跨境案件)期间的合作程度？

英文题干：Which of the following best describes the degree of cooperation between regulatory authorities, law enforcement agencies, and Financial Intelligence Units (FIUs) during money laundering investigations, including cross-border cases?

选项：

- A. 监管机构主要独立开展工作,但应国际执法机构的要求会共享信息.
  English: Regulatory authorities primarily work independently but share information when requested by international law enforcement agencies.
- B. 金融情报机构主要处理金融机构提交的报告,而监管机构和执法部门则仅限于执行国内反洗钱法律,无法开展国际合作.
  English: FIUs primarily handle reporting from financial institutions, while regulatory authorities and law enforcement agencies are limited to enforcing domestic AML laws without international cooperation.
- C. 执法机构和金融情报机构仅在国内调查期间开展合作,而将跨境调查交由国际刑警组织等国际组织负责.
  English: Law enforcement agencies and FIUs only cooperate during domestic investigations, leaving cross-border investigations to international organizations like Interpol.
- D. 监管机构、执法部门和金融情报机构通过正式协议共享情报并协调行动,以简化跨境洗钱调查工作.
  English: Regulatory authorities, law enforcement agencies, and FIUs share intelligence through formal agreements and coordinate actions to streamline cross-border money laundering investigations.

## 【AI答案】

D

## 【考点】

理解监管、执法与FIU在跨境调查中的正式合作机制

## 【核心解析】

公私部门当局之间的国际合作是打击跨国金融犯罪的关键。执法人员、监管机构和国家金融情报机构(FIU)不仅建立战略伙伴关系，还在个案层面开展合作(P229)。各种机构模型下，各方都必须协同工作并在涉及彼此职能时共享信息(P229)。因此，题干所描述的三方通过正式协议共享情报、协调行动以简化跨境洗钱调查，正是教材框架下合作程度最准确的概括。

教材原句："International cooperation among public sector authorities is key to fighting transnational financial crime. Law enforcement, regulators, and national FIUs develop strategic partnerships as well as cooperate at a case level."

## 【错误项分析】

- **A 错误（教材直接依据）｜范围或程度偏差**：教材表明监管机构不仅在境内合作，也在国际层面合作并主动共享信息(P230-231)，而A将其描述为“主要独立开展工作”且仅应国际请求才共享信息，与教材中主动、持续的协调合作框架相比，匹配度明显更低。
- **B 错误（教材直接依据）｜范围或程度偏差**：FATF建议明确要求当局通过信息交换和联合调查促进国际合作(P149)，打击洗钱本身就需要国内与国际合作及信息共享(P233)。B将监管机构和执法部门限缩为“无法开展国际合作”，与教材多处强调的国际协作框架直接矛盾。
- **C 错误（教材直接依据）｜范围或程度偏差**：根据FATF标准和埃格蒙特集团原则，FIU应自发或应请求相互传播金融情报(P231)，并可将其纳入操作分析后传播给执法部门采取行动，这意味着FIU与执法机构的合作本身就涵盖跨境维度。C将其合作限定为“仅在国内调查期间”不符合教材描述的跨境情报共享机制。

## 【易错提醒】

（无）

---

教材章节：未映射

题型：multiple

题干：政府金融情报机构(FIU)的职责有哪些？(选择三项.)

英文题干：What are the roles of a government Financial Intelligence Unit (FIU)? (Select three.)

选项：

- A. 向外国司法系统传播可疑交易和可疑活动报告的分析结果,以增强其反洗钱和反恐融资的调查及起诉工作.
  English: Disseminate analysis of suspicious transaction and suspicious activity reports to foreign judicial systems to enhance their anti-money laundering and terrorist financing investigations and prosecutions.
- B. 接收报告机构或义务机构提交的可疑交易和可疑活动报告
  English: Receive reports of suspicious transactions and suspicious activities from reporting institutions or obliged institutions.
- C. 向当地执法机构和外国金融情报机构传播可疑交易和可疑活动报告的分析结果,以打击洗钱活动
  English: Disseminate the analysis of suspicious transaction and suspicious activity reports to local law enforcement agencies and foreign FIUs to combat money laundering.
- D. 对从报告机构或义务机构收到的所有可疑交易和可疑活动报告进行调查,并在适当情况下提起诉讼
  English: Investigate all suspicious transaction and suspicious activity reports received from reporting institutions or obliged institutions and prosecute cases where appropriate.
- E. 分析从报告机构或义务机构收到的所有可疑交易和可疑活动报告
  English: Analyze all suspicious transaction and suspicious activity reports received from reporting institutions or obliged institutions.

## 【AI答案】

B、C、E

## 【考点】

FIU 接收、分析、传播可疑报告的三大核心职能

## 【核心解析】

国家金融情报机构（FIU）的核心职能是接收、分析和传播金融情报（P228）。题干问FIU的职责，就是要找同时具备这三个动作的选项。选项B“接收”报告实体提交的可疑报告，选项E“分析”这些报告，选项C将分析结果“传播”给国内执法机构和外国FIU——三项合在一起，恰好完整覆盖了教材对FIU的职能定义。FATF也要求各辖区设立FIU来完成这三件事（P231），三者属于同一组不可分割的职责链条。

教材原句："National FIUs receive, analyze, and disseminate financial intelligence."

## 【错误项分析】

- **A 错误（教材定义应用）｜范围或程度偏差**：教材指出FIU应向外国FIU传播金融情报（P231），而非直接向“外国司法系统”传播。选项A用“外国司法系统”替代了“外国FIU”，传播对象发生了偏移，不如选项C中“外国金融情报机构”的表述更为准确。
- **D 错误（教材定义应用）｜主体或阶段错配**：调查和起诉是执法机构的职责而非FIU的职责。教材说明FIU进行的是操作分析，目的是为执法机构提供可用于调查的情报（P231），FIU传播的情报包有时仅供情报使用，不能直接作为法庭证据（P232）。选项D将执法职能归入FIU，不如B、C、E更吻合教材对FIU“情报枢纽”而非“执法主体”的定位。

## 【易错提醒】

FIU是情报中转站，不是执法行动队。FIU自己不去抓人、不起诉，它把分析好的情报交给执法机构去执行（P231）。题目里一旦出现“调查”“起诉”“逮捕”这类词，大概率是执法机构的活，不是FIU的。

---

教材章节：未映射

题型：multiple

题干：一个国家金融情报机构(FIU)的主要职责包括:(选择两项.)

英文题干：The main responsibilities of a national Financial Intelligence Unit (FIU) include: (Select two.)

选项：

- A. 与执法机构、金融机构及其他利益相关方合作,以发现并阻止非法金融活动.
  English: Collaborating with law enforcement agencies, financial institutions, and other stakeholders to detect and prevent illicit financial activities.
- B. 对金融机构进行检查,以确保其遵守反洗钱规定.
  English: Conducting examinations of financial institutions to ensure compliance with anti-money laundering regulations.
- C. 从事向公共和私营部门提供金融服务,包括银行业务和投资活动
  English: Engaging in providing financial services, including banking and investment activities, to the public and private sectors.
- D. 促进公共部门与私营部门之间的信息交流.
  English: Facilitating the exchange of information between the public and private sectors.
- E. 制定有关金融机构运营的立法.
  English: Enacting legislation regarding the operations of financial institutions.

## 【AI答案】

A、D

> **需人工复核**
>
> - 盲判校验未通过

## 【考点】

区分FIU职责与监管机构、立法机构职责

## 【核心解析】

国家金融情报机构（FIU）的核心职能是接收、分析和传播金融情报（P228），其设立目的即分析可疑交易报告并支持执法调查（P149）。由此可推断，FIU的职责本质上是一个“信息枢纽”——它自己不直接执法、不直接监管、不从事商业金融活动、也不立法。选项A“与执法机构、金融机构及其他利益相关方合作，以发现并阻止非法金融活动”直接对应FIU接收分析情报后传播给执法机构促成执法行动这一链条（P232）。选项D“促进公共部门与私营部门之间的信息交流”对应FIU作为信息枢纽在公私之间共享金融情报的职能，教材明确指出公私合作伙伴关系可用于促进公共和私营部门合作打击金融犯罪（P237）。两项均落在FIU“信息收集—分析—分发”的核心职能辐射范围内。

教材原句："National FIUs receive, analyze, and disseminate financial intelligence."

## 【错误项分析】

- **B 错误（教材定义应用）｜概念混淆**：“对金融机构进行检查以确保其遵守反洗钱规定”是监管机构的职责，而非FIU。教材在韩国案例中明确指出：金融监督院（Financial Supervisory Service）负责检查和监督金融机构的合规情况，而韩国金融情报中心（Korea FIU）分析可疑交易报告并与执法机构共享信息（P203）。FIU的角色定位是情报分析而非合规检查，题干更直接匹配A和D而非B。
- **C 错误（题干对照）｜题干要素不匹配**：“从事向公共和私营部门提供金融服务”是商业银行等金融机构的业务活动（P64），与FIU的公共职能完全无关。FIU是政府设立的情报机构，不从事商业银行业务或投资活动。题干要素不匹配。
- **E 错误（题干对照）｜概念混淆**：“制定有关金融机构运营的立法”是立法机关的职责。FIU本身是依据法律设立的机构，执行法律授权范围内的情报职能，而非制定法律。教材提到各辖区应通过立法对机构施加义务（P43），但FIU是守法者/执行者而非立法者。题干更匹配A和D。

## 【易错提醒】

FIU的核心定位是“信息枢纽”而非“执法者”或“监管者”。教材原文：FIU接收、分析和传播金融情报（P228）；对金融机构的检查监督由监管机构（如金融监督院）执行（P203）。判断口诀——FIU管信息，监管管检查，执法管抓人。

---

